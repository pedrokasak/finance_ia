...
test.describe('Authentication', () => {
    ...
    test('mostra erro com credenciais inválidas', async ({ page }) => {
        ...
        await expect(page.getByText(/inválid|invalid|credencial/i)).toBeVisible({ timeout: 5000 });
    });

    test('navega para signup', async ({ page }) => {
        await page.goto('/');
        await page.getByText(/criar conta|sign up|cadastrar/i).click();
        await expect(page.getByRole('heading', { name: /criar conta|cadastro|sign up/i })).toBeVisible();
    });
