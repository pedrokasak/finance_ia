...
export function OnboardingFlow({ onComplete }: OnboardingFlowProps) {
    ...
    const handleComplete = async () => {
        ...
        try {
            ...
            const needs = split.find((s) => ['Necessidades', 'Fixos', 'Gastos', 'Despesas Fixas', 'Moradia', 'Viva com o resto'].includes(s.label));

            const wants = split.find((s) => ['Desejos', 'Variáveis', 'Alimentação', 'Lazer e Metas'].includes(s.label));
            const savings = split.find((s) => ['Investimentos', 'Reservas', 'Poupança', 'Investimento Imediato', 'Investimentos (30%+)'].includes(s.label));
                ...
                <DialogContent className="max-w-md">
                    ...
                    {detailMethod && (
                        <div className="space-y-4">
                            ...
                            <Button className="w-full" onClick={() => { setSelectedMethod(detailMethod); setDetailMethod(null); }}>
                ...
                <div className="flex items-center gap-2 mb-8">
                    {Array.from({ length: totalSteps }).map((_, i) => (
                        <div key={i} className="flex-1">
                            <div className={`h-1.5 rounded-full transition-all duration-500 ${i + 1 <= step ? 'bg-primary' : 'bg-muted'}`} />
                        </div>
                    ))}
                    <span className="text-xs text-muted-foreground whitespace-nowrap">{step}/{totalSteps}</span>
                </div>
                ...
                {step === 1 && (
                    <div className="text-center space-y-6 animate-in fade-in">
                        <div className="space-y-2">
                            ...
                            <h1 className="text-3xl font-bold tracking-tight">Bem-vindo ao FinanceIA</h1>
                    ...
                    <div className="space-y-6 animate-in fade-in">
                        <div>
                            ...
                            <p className="text-muted-foreground mt-1">
                                Clique em <Info className="inline h-3 w-3" /> para saber mais sobre cada método.
                            </p>
                        </div>
                        {methodsLoading ? (
                ...
                {step === 4 && selectedMethod && (
                    <div className="space-y-6 animate-in fade-in">
                        <div>
                            ...
                            <p className="text-muted-foreground mt-1">
                                Com base na <strong>{selectedMethod.name}</strong>, calcularemos a distribuição ideal.
                            </p>
                        </div>
                        <div className="space-y-4">
                            ...
                            {monthlyIncome && parseFloat(monthlyIncome) > 0 && (
                                <Card className="animate-in fade-in">
                                    <CardContent className="p-4 space-y-3">
                                        <p className="text-sm font-medium text-muted-foreground">Sua distribuição com {selectedMethod.name}:</p>
                                        {selectedMethod.split.map((s) => {
                                            const amount = (parseFloat(monthlyIncome) * s.percent) / 100;
                                            return (
